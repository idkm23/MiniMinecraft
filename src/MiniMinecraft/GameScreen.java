...
public class GameScreen extends JPanel {
    ...
    // font metrics for the back button is stored to make an accurate hit box for the back button
    private FontMetrics bButtonFM;

    @Override
    public void paintComponent(Graphics g) {
        super.paintComponent(g);
        // sky color changes based on the time
        setBackground(getSky());
        ...
        drawUI(g);
        drawSelectionBox(g);
        // makes the game appear to get darker when it's night
        drawNightTint(g);
    ...
    private void drawBackButton(Graphics g) {
        ...
        g.setFont(new Font(
                    "Monospaced",
                    (onBackButton(Map.getMousePos()) ?  Font.ITALIC: Font.PLAIN ),
                    20));

        bButtonFM = g.getFontMetrics();
        g.drawString("Return to main menu",
                getWidth() - bButtonFM.stringWidth("Return to main menu") - 20,
                bButtonFM.getAscent() + 20);
    }
    ...
    public boolean onBackButton(Point mouse) {
        ...
        return (mouse.y < bButtonFM.getAscent() + 20
                && mouse.x > Map.canvas.getWidth() - bButtonFM.stringWidth("Return to main menu") - 20
                && mouse.y > 20 && mouse.x < Map.canvas.getWidth() - 20);
    }

    // draws the moon and sun, revolving on the screen based on the time of day
    private void drawMoonSun(Graphics g) {
        ...
        g.drawImage(images[Map.MOON].getImage(),
                (int) (getWidth()/2 + getWidth()/2 * Math.cos(theta)) - Map.BSIZE,
                (int) (getHeight() + getHeight() * Math.sin(theta)) - Map.BSIZE,
                Map.BSIZE * 2, Map.BSIZE * 2,
                this);
    }

    private void drawClouds(Graphics g) {
        for(Cloud c : Map.prettyClouds) {
            g.drawImage(images[c.cloudID].getImage(),
                    (int) c.getX() + Map.hOffset,
                    (int) c.getY() + Map.vOffset,
                    images[c.cloudID].getIconWidth(),
                    images[c.cloudID].getIconHeight(),
                    this);
        }
    }
    ...
    private void drawEntities(Graphics g) {
        for(Entity e : Map.entityList) {
            g.drawImage(images[e.imageID].getImage(),
                    (int) (e.screenX() + (e.getOrientation() ? Map.BSIZE * e.getWidth() : 0)),
                    (int) e.screenY(),
                    (e.getOrientation() ? -1 : 1) * (int)(Map.BSIZE * e.getWidth()),
                    (int) (Map.BSIZE * e.getHeight()),
                    this);
        }
    }
    ...
    private void drawPlayer(Graphics g) {
        ...
        g.drawImage(images[Map.player.imageID].getImage(),
                (int) (Map.player.x + (Map.player.getOrientation() ? Map.BSIZE * Map.player.getWidth() : 0)), // draws player
                (int) Map.player.y,
                (int) ((Map.player.getOrientation() ? -1 : 1) * Map.BSIZE * Map.player.getWidth()),
                (int) (Map.BSIZE * Map.player.getHeight()),
                this);
    ...
    private void drawUI(Graphics g) {
        for(int i = 8; i >= 1; i--) {
            g.drawImage(
                    images[(Map.player.getHealth() < i) ?
                            Map.EMPTY_HEALTH_IMG : Map.FULL_HEALTH_IMG].getImage(),
                    getWidth() - 35 * i - 20,
                    getHeight() - 50, 30, 30, this);
        }

        g.drawImage(images[Map.INVENTORY_BAR].getImage(),
                25,
                getHeight() - images[Map.INVENTORY_BAR].getIconHeight() * 3 - 10,
                images[Map.INVENTORY_BAR].getIconWidth() * 3,
                images[Map.INVENTORY_BAR].getIconHeight() * 3, this);
        ...
        for(int i = 0; i < itemBar.length; i++) {
            ...
            if(i == Map.player.pInven.getSelectedIndex()) {
                    ...
                    g.drawRect((images[Map.INVENTORY_BAR].getIconHeight() - 2) * 3 * i + 27,
                            getHeight() - (images[Map.INVENTORY_BAR].getIconHeight() - 2) * 3 - 15, 
                            (images[Map.INVENTORY_BAR].getIconHeight() - 2) * 3,
                            (images[Map.INVENTORY_BAR].getIconHeight() - 1) * 3);
                    g.setColor(Color.WHITE);
            }
        }
    }
