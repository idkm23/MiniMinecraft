...
public class Block {
    public static final int DIRT = 0, STONE = 1, GRASS = 2, BEDROCK = 3, WOOL = 4, PLAYER_HEAD = 5;
    ...
    public boolean obstructsActor() {
        ...
        for(Entity e : Map.entityList) {
            if(!(e instanceof Actor))
                continue;
            diffX = x - e.x;
            diffY = y - e.y;        //the if statement checks for a collison between the block and the entity
            if(-Map.BSIZE < diffX && diffX <= e.getWidth() * Map.BSIZE && Map.BSIZE * e.getHeight() >= diffY && diffY >= -Map.BSIZE ) 
                return true; //if there was a collison return true
            
        }
        
        //this checks if the PLAYER is going to be obstructed by our new block using a similar process
